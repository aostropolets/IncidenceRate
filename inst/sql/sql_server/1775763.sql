...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (312653,4331309,4143969,320127,4231274,317307,320745,317301,444083,321578,442569,319728,319720,443778)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (312653,4331309,4143969,320127,4231274,317307,320745,317301,444083,321578,442569,319728,319720,443778)
...
WHERE (YEAR(C.condition_start_date) - P.year_of_birth >= 0 and YEAR(C.condition_start_date) - P.year_of_birth <= 100)
AND C.ordinal = 1
